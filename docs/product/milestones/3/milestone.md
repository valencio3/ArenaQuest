...
* **Quality Gate:** a first pass at E2E tests (Playwright) — long deferred from Milestone
  2 — covering the "create topic → publish → student sees it" happy path.
...
* Playwright E2E scaffold under `e2e/` with a single smoke test: login as admin →
  create topic → publish → log in as student → topic visible in catalogue.
...
* [ ] An admin can create a root topic and a nested child topic via the UI; both
      persist across reloads.
* [ ] A topic can be moved to a new parent via drag-drop; the order persists.
* [ ] A circular move (A → descendant of A) returns `409 Conflict` and the UI surfaces
      the error without corrupting state.
* [ ] An admin can upload a PDF and an MP4 to a topic using presigned URLs; the upload
      bypasses the Worker (observable via the Network tab — direct POST to R2).
* [ ] The "upload progress" bar updates in real time (uses `XMLHttpRequest.upload` or
      `fetch` stream).
* [ ] Media larger than the type limit is rejected before upload.
* [ ] PENDING media is not listed on the student surface.
* [ ] A topic in `draft` status is not visible to a student; publishing it makes it
      visible within one request.
* [ ] A student can open the catalogue, navigate the tree, and see Markdown rendered
      without script execution when the source contains `<script>` tags (XSS test).
* [ ] A student can open a PDF inline and stream an MP4 in the same page.
* [ ] `DELETE /admin/topics/:id` cascades to archive descendants; media remains
      accessible via the admin surface but hidden from students.
* [ ] Deleting a media row also removes the underlying storage object (verify by
      listing the R2 bucket before and after).
* [ ] Playwright smoke test passes in CI.
* [ ] All Vitest suites (api + web) remain green; `make lint` clean.
* [ ] `make cf-typegen` still produces a clean types file after the new binding is
      added.
...
| # | Task File | Status |
|---|-----------|--------|
| 01 | [Topics & Tags Data Layer (port + D1 adapter + migration)](./01-topics-data-layer.task.md) | ⬜ Pending |
| 02 | [Media Data Layer (entity + migration + linking)](./02-media-data-layer.task.md) | ⬜ Pending |
| 03 | [R2 Storage Adapter (implement IStorageAdapter)](./03-r2-storage-adapter.task.md) | ⬜ Pending |
| 04 | [Admin Topics CRUD + Move API](./04-admin-topics-crud-api.task.md) | ⬜ Pending |
| 05 | [Admin Media API (presigned upload + finalize + delete)](./05-admin-media-api.task.md) | ⬜ Pending |
| 06 | [Public Topics Read API (published-only catalogue)](./06-public-topics-read-api.task.md) | ⬜ Pending |
| 07 | [Markdown Sanitization Helper (shared package)](./07-markdown-sanitization.task.md) | ⬜ Pending |
| 08 | [Frontend: Admin Topic Tree Dashboard](./08-frontend-admin-topic-tree.task.md) | ⬜ Pending |
| 09 | [Frontend: Admin Media Uploader (presigned flow + progress)](./09-frontend-admin-media-uploader.task.md) | ⬜ Pending |
| 10 | [Frontend: Student Catalogue + Content Viewers](./10-frontend-student-catalogue.task.md) | ⬜ Pending |
| 11 | [E2E Scaffolding (Playwright smoke test)](./11-e2e-playwright-scaffold.task.md) | ⬜ Pending |
...
* [ ] All 15 milestone-level acceptance criteria in §3 pass.
* [ ] `make lint`, `make test`, and `make e2e` (newly introduced) are green in CI.
* [ ] The demo walk-through works end-to-end on a fresh deploy: admin logs in, builds a
...
* [ ] `docs/ReleaseNotes.md` gains a **Milestone 3 — Content & Media Core** section
      summarising entities, endpoints, and UI surfaces added.
...
* [ ] The agnosticism contract still holds: `grep -R "R2Bucket\|@aws-sdk" apps/api/src`
      returns hits **only** inside `apps/api/src/adapters/storage/`; nothing in `core/`,
      `controllers/`, `routes/`, or `middleware/`.
* [ ] No regression on Milestone 2 hardening: the security findings closed in
